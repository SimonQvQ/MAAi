...
def _run_loop(server: AgentServer, entry: str, resource_path: str):
    """等待 agent 拨入后，用 maa_controller 跑任务。"""
    ...
    while True:
        ...
        if agent:
            log(f"[run] 使用 agent {agent.addr} 执行 {entry}")
            try:
                detail = run_agent_task(agent, resource_path, entry)
...
def main():
    ...
    if args.run:
        ...
        _run_loop(server, args.entry, args.resource)
    else:
        server.start()
