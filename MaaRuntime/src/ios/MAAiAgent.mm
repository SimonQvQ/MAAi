...
+ (void)maybeAutoStart {
  ...
  if (!host) return; // 未配置环境变量不自动连（由宿主进程调用 C ABI 或注入后手动触发）
  NSString* portStr = GetEnv(@"MAAI_SERVER_PORT", @"17171");
  uint16_t port = (uint16_t)[portStr intValue];
  [[MAAiAgent shared] startWithHost:host port:port];
}
